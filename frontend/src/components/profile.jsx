...
class Profile extends Component {
  ...
  componentDidMount() {
    ...
    this.fetchbookLoans(userdata.id);
  }
  ...
  fetchbookCopies = async (bookLoans) => {
    try {
      ...
      const uniqueBookCopyIds = [
        ...new Set(bookLoans.map((loan) => loan.copyBookId)),
      ];
      ...
      const bookCopies = await Promise.all(
        uniqueBookCopyIds.map(async (copyBookId) => {
          try {
            const response = await fetch(
              `http://localhost:8080/bookcopies/id/${copyBookId}`
            );
            const data = await response.json();
      ...
      const mergedBookLoans = bookLoans.map((loan) => {
        const matchingBookCopy = bookCopies.find(
          (copy) => copy.id === loan.copyBookId
        );
        return {
          ...loan,
  ...
  fetchbooks = async (mergedBookLoans) => {
    try {
      ...
      const uniqueIsbns = [
        ...new Set(mergedBookLoans.map((copy) => copy.isbn)),
      ];

      // Książki dla każdego unikalnego isbn
      Promise.all(
        uniqueIsbns.map((isbn) =>
          fetch(`http://localhost:8080/books/id/${isbn}`).then((response) =>
            response.json()
          )
        )
      ).then((books) => {
